...
#' @include mcpr-server-forwarding.R
...
mcprServer <- R6::R6Class("mcprServer",
  ...
  private = list(
    ...
    # Delegate dropped-worker pipe detection to the forwarding lifecycle module.
    daemon_pipe_dropped = function(client_id) {
      mcpr_forwarding_daemon_pipe_dropped(private, client_id)
    },
    ...
    # Delegate attached-session response correlation to the forwarding lifecycle module.
    handle_message_from_session = function(data, session_key = NULL) {
      mcpr_forwarding_handle_message(private, data, session_key)
    },
    ...
    # Delegate forwarded-request validation to the forwarding lifecycle module.
    prepare_forward_request = function(data) {
      mcpr_forwarding_prepare_request(private, data)
    },

    # Delegate active-request transmission to the forwarding lifecycle module.
    send_active_request = function(session_key, sock, label = "TO TARGET") {
      mcpr_forwarding_send_active(private, session_key, sock, label)
    },

    # Delegate bounded FIFO admission to the forwarding lifecycle module.
    enqueue_pending_request = function(data, session_key) {
      mcpr_forwarding_enqueue(private, data, session_key)
    },

    # Delegate waiting-request promotion to the forwarding lifecycle module.
    dispatch_next = function(session_key, sock) {
      mcpr_forwarding_dispatch_next(private, session_key, sock)
    },

    # Delegate current-socket lookup to the forwarding lifecycle module.
    session_socket_for = function(session_key) {
      mcpr_forwarding_session_socket(private, session_key)
    },

    # Delegate dead-session reporting to the forwarding lifecycle module.
    fail_pending_dead = function(record, session_key) {
      mcpr_forwarding_fail_dead(private, record, session_key)
    },

    # Delegate send-failure reporting to the forwarding lifecycle module.
    fail_pending_send = function(record, session_key, reason) {
      mcpr_forwarding_fail_send(private, record, session_key, reason)
    },

    # Delegate failed-transport retirement to the forwarding lifecycle module.
    retire_session_transport = function(session_key) {
      mcpr_forwarding_retire_transport(private, session_key)
    },

    # Delegate listener completion handling to the forwarding lifecycle module.
    handle_session_listener_resolved = function(data, session_key, session_type) {
      mcpr_forwarding_handle_listener(private, data, session_key, session_type)
    },

    # Delegate timeout and asynchronous-send sweeps to the forwarding lifecycle module.
    sweep_pending_requests = function() {
      mcpr_forwarding_sweep_pending(private)
    },
