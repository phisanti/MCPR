  ...
  public = list(
    ...
    state_get = function(key, default = NULL) {
      ...
      if (exists(key, envir = the) && !is.null(the[[key]])) {
        value <- the[[key]]
        private$.logger$debug(sprintf("STATE_GET: %s -> %s", key, 
          if (is.null(value)) "NULL" else class(value)[1]))
        return(value)
      } else {
        private$.logger$debug(sprintf("STATE_GET: %s -> DEFAULT (%s)", key, 
    ...
    state_set = function(key, value) {
      ...
      private$.logger$debug(sprintf("STATE_SET: %s <- %s", key, 
        if (is.null(value)) "NULL" else class(value)[1]))
      invisible(self)
    },
    ...
    cleanup_all = function() {
      ...
      private$.logger$debug(sprintf("CLEANUP_START: %d resources, %d state keys", 
