...
#' Create and view R plots with intelligent token management
#'
#' This is the primary tool for data visualization in MCPR. Execute R plotting 
#' code and immediately see the results. Features intelligent token estimation,
#' optimization warnings, and agent-optimized defaults based on experimental data.
#' Supports all standard R plotting including base plots, ggplot2, plotly, and 
#' other graphics packages.
#'
#' @param expr R code expression to generate the plot 
#' @param width Width of the output image in pixels (default: 600, optimized for AI agents)
#' @param height Height of the output image in pixels (default: 450, optimized for AI agents)  
...
#' @param warn_threshold Token count threshold for optimization warnings (default: 20000)
#' @keywords mcpr_tool
...
create_plot <- function(expr, width = 600, height = 450, format = "png", 
  ...
  tryCatch({
    ...
    if (actual_tokens > token_limit) {
      ...
      error_msg <- sprintf(
        "Plot size (%s tokens) exceeds token limit (%s tokens).\n\nOptimization suggestions:\n- %s\n\nNote: This plot is intended for AI analysis, not human presentation. Consider using smaller dimensions for better efficiency.",
