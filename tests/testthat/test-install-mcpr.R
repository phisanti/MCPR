...
test_that("install_mcpr validates other arguments correctly", {
  ...
  # Invalid server_name
  expect_error(install_mcpr("claude", server_name = c("a", "b")), 
               "must be a single string")
  expect_error(install_mcpr("claude", server_name = NULL), 
               "must be a single string")
  expect_error(install_mcpr("claude", server_name = NA_character_), 
               "must be a single string")
  
  # Invalid force
  expect_error(install_mcpr("claude", force = "yes"), 
               "must be a single logical value")
  expect_error(install_mcpr("claude", force = c(TRUE, FALSE)), 
               "must be a single logical value")
})
