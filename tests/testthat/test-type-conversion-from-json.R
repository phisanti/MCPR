...
test_that("from_mcp_json reconstructs special numeric values", {
  # Test round-trip with special numeric values
  original <- c(1.5, Inf, -Inf, NaN)
  json_data <- to_mcp_json(original)
  result <- from_mcp_json(json_data)
...
test_that("from_mcp_json handles nested structures", {
  ...
  result <- from_mcp_json(nested_json)
...
test_that("from_mcp_json handles invalid JSON and empty structures gracefully", {
  # Test invalid JSON string
  expect_error(from_mcp_json('{"invalid": json}'))
  
  # Test empty structures
  expect_equal(from_mcp_json('{}'), structure(list(), names = character(0)))
  expect_equal(from_mcp_json('[]'), list())
  expect_equal(from_mcp_json('""'), "")
  
  # Test empty MCP type objects
  empty_matrix <- list(`_mcp_type` = "matrix", data = numeric(0), dim = c(0, 0))
  result <- from_mcp_json(empty_matrix)
...
test_that("from_mcp_json handles malformed MCP type markers", {
  # Test unknown MCP type
  unknown_type <- list(`_mcp_type` = "unknown_type", data = "test")
  result <- from_mcp_json(unknown_type)
...
test_that("from_mcp_json reconstructs complex S3 objects with nested MCP types", {
  ...
  result <- from_mcp_json(s3_object)
...
test_that("from_mcp_json reconstructs data frames with complex column types", {
...
test_that("from_mcp_json reconstructs complex multi-dimensional arrays", {
  ...
  result <- from_mcp_json(array_json)
