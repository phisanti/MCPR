test_that("process_tool_arguments_enhanced handles basic arguments", {
  # Test basic argument passthrough
  basic_args <- list(x = 42, y = "test", z = TRUE)
  result <- process_tool_arguments_enhanced(basic_args)
  expect_equal(result, basic_args)
  
  # Test non-list input
  expect_equal(process_tool_arguments_enhanced("not a list"), "not a list")
  expect_equal(process_tool_arguments_enhanced(42), 42)
})

test_that("process_tool_arguments_enhanced detects MCP type markers", {
  ...
  result <- process_tool_arguments_enhanced(mcp_args)
  expect_true(is.list(result))
  # The exact structure depends on from_mcp_json implementation
  expect_true(!is.null(result))
})

test_that("process_tool_arguments_enhanced handles empty and NULL inputs", {
  # Test NULL
  expect_null(process_tool_arguments_enhanced(NULL))
  
  # Test empty list
  expect_equal(process_tool_arguments_enhanced(list()), list())
  
  # Test list without MCP markers
  no_markers <- list(a = 1, b = list(x = 2, y = 3))
  expect_equal(process_tool_arguments_enhanced(no_markers), no_markers)
})

test_that("as_tool_call_result_enhanced handles simple text results", {
  # Test simple character result
  test_data <- list(id = 1)
  simple_result <- "Simple text result"
  
  output <- as_tool_call_result_enhanced(test_data, simple_result)
...
test_that("as_tool_call_result_enhanced handles complex objects", {
  ...
  output <- as_tool_call_result_enhanced(test_data, complex_result)
...
test_that("as_tool_call_result_enhanced handles numeric vectors", {
  # Test numeric vector (should trigger mcp_serialize)
  test_data <- list(id = 3)
  numeric_result <- c(1, 2, 3, 4, 5)
  
  output <- as_tool_call_result_enhanced(test_data, numeric_result)
...
test_that("as_tool_call_result_enhanced handles lists", {
  # Test list result
  test_data <- list(id = 4)
  list_result <- list(status = "success", data = 1:5, message = "Complete")
  
  output <- as_tool_call_result_enhanced(test_data, list_result)
