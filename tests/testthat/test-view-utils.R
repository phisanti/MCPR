...
test_that("capture_print captures object output", {
  result <- capture_print(1:5, max_print = 10)
...
test_that("format_file_size formats sizes correctly", {
  expect_equal(format_file_size(100), " (100 bytes)")
  ...
  expect_equal(format_file_size(NA), "")
})

test_that("format_count formats counts correctly", {
  expect_equal(format_count(0, "item"), "No items")
  expect_equal(format_count(1, "item"), "1 item")
  expect_equal(format_count(5, "item"), "5 items")
  expect_equal(format_count(20, "item", 10), "20 items (showing first 10)")
})

test_that("build_result builds formatted results", {
  result <- build_result("Title", "Line 1", "Line 2")
  expected <- "Title\nLine 1\nLine 2"
  expect_equal(result, expected)

  # Test with no content
  result2 <- build_result("Title Only")
  expect_equal(result2, "Title Only")
})

test_that("safe_eval handles errors gracefully", {
  # Test successful evaluation
  result1 <- safe_eval(2 + 2, "fallback", TRUE)
  expect_equal(result1, 4)

  # Test error handling with error message
  result2 <- safe_eval(stop("test error"), "fallback", TRUE)
  expect_equal(result2, "fallback: test error")

  # Test error handling without error message
  result3 <- safe_eval(stop("test error"), "fallback", FALSE)
  expect_equal(result3, "fallback")
})

test_that("truncate_text truncates properly", {
  short_text <- "short"
  expect_equal(truncate_text(short_text, 10), "short")
...
test_that("is_likely_text_file identifies file types correctly", {
  ...
  expect_true(is_likely_text_file("test.txt", "txt"))
  expect_true(is_likely_text_file("test.R", "R"))
  expect_true(is_likely_text_file("test.md", "md"))
  expect_true(is_likely_text_file("test.json", "json"))
  expect_true(is_likely_text_file("test.py", "py"))
  expect_true(is_likely_text_file("test.css", "css"))

  # Function should always return a logical value
  expect_type(is_likely_text_file("test.txt", "txt"), "logical")
})

test_that("capture_print handles different object types", {
  # Test with data frame
  df_result <- capture_print(data.frame(x = 1:3, y = letters[1:3]))
  expect_type(df_result, "character")
  expect_true(any(grepl("x", df_result)))
  
  # Test with list
  list_result <- capture_print(list(a = 1, b = "hello"))
  expect_type(list_result, "character")
  
  # Test with function
  func_result <- capture_print(function() "test")
  expect_type(func_result, "character")
})

test_that("capture_print respects max_print parameter", {
  long_vector <- 1:1000
  result_short <- capture_print(long_vector, max_print = 10)
  result_long <- capture_print(long_vector, max_print = 200)
...
test_that("format_error_info cleans error messages", {
  ...
  result <- format_error_info(test_error)
...
test_that("get_file_content_preview works with text files", {
  ...
  result <- get_file_content_preview(temp_file, max_lines = 3)
...
test_that("get_file_content_preview handles empty files", {
  ...
  result <- get_file_content_preview(temp_file, max_lines = 5)
...
test_that("is_likely_text_file handles unknown extensions", {
  ...
  result <- is_likely_text_file(temp_file, "")
...
test_that("build_result handles NULL inputs", {
  result <- build_result("Title", NULL, "Valid line", NULL)
...
test_that("utility functions handle edge cases", {
  ...
  expect_equal(format_count(0, "test"), "No tests")
  expect_equal(build_result("Test"), "Test")
  expect_equal(truncate_text("", 10), "")
  
  # Test error handling
  expect_no_error(safe_eval(NULL, "fallback"))
  expect_no_error(format_error_info(list()))
})
